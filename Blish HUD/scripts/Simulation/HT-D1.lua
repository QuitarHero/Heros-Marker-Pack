...
function HTcm_D1_VoidPoolBreath(curTime, startTime, interval, table, index)
  ...
  if(not table[index]) then
    for i = 1, #vpBreath.markers do
      if( i % 2 == 0 ) then
        vpBreath.markers[i].Tint = HMP.HTcmSim.color[1]
      else
        vpBreath.markers[i].Tint = HMP.HTcmSim.color[3]
      end
    end
...
function HTcm_D1_GraspOfJormag(curTime, startTime, interval, table, index)
  ...
  for i = 1, #grasp.markers do
    ...
    if(grasp.track[a] == 3) then
      ...
      grasp.pos[2][a] = HMPphysics.pos.randomXYedge(grasp.markers[i].Position, grasp.radius)
      grasp.track[a] = 4
    end
